...
#	echo $sub_ID $sub_key
	...
	for the_run in $(ls $source_dir/*$sub_ID*/*/ |grep PI_DiCo)
	do
...
#		echo $n_files
		if [ $n_files == 175 ]
		then
			mkdir -p data/sub-$sub_key/ses-cogtasks/func
#			echo $sub_ID $sub_key $the_run r0$run_counter
			
		   	dcm2niix -o ./data/sub-$sub_key/ses-cogtasks/func -f sub-$sub_key"_task-percim_run-0"$run_counter"_bold" -z y $source_dir/*$sub_ID*/*/$the_run/*.DCM
